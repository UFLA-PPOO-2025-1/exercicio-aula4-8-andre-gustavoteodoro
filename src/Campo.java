...
public class Campo {
    ...
    public List<Localizacao> todasLocalizacoesLivres(Localizacao ignorar) {
        List<Localizacao> livres = new LinkedList<>();
        for (int linha = 0; linha < comprimento; linha++) {
            for (int coluna = 0; coluna < largura; coluna++) {
                Localizacao loc = new Localizacao(linha, coluna);
                if ((ignorar == null || !loc.equals(ignorar)) && obterObjetoEm(loc) == null) {
                    livres.add(loc);
                }
            }
        }
    ...
    public Localizacao localizacaoAleatoriaLivre(Localizacao ignorar) {
        List<Localizacao> livres = todasLocalizacoesLivres(ignorar);
        if (!livres.isEmpty()) {
